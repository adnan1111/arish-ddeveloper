...
');
	...
	function admin_footer() {
		printf(__('%1$s | %2$s %3$s by %4$s', 'seo-wizard'),
			...
			'<a href="'.SU_PLUGIN_URI.'" target="_blank" rel="nofollow">'.__(SU_PLUGIN_NAME, 'seo-wizard').'</a>',
			SU_VERSION,
			'<a href="'.SU_AUTHOR_URI.'" target="_blank" rel="nofollow">'.__(SU_AUTHOR, 'seo-wizard').'</a>'
		);
...
<?php
	...
	function textboxes($textboxes, $defaults=array(), $grouptext=false, $args=array(), $textbox_args=array()) {
		...
		foreach ($textboxes as $id => $title) {
			...
			$id = su_esc_attr($id);
			$resetmessage = su_esc_attr(__('Are you sure you want to replace the textbox contents with this default value?', 'seo-wizard'));
